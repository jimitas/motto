function createMokkinAudioSources() {
  ...
  for (let i = 0; i < No.length; i++) {
    if (No[i] !== 0) {
      promises.push(new Promise((resolve) => {
        se[No[i]] = new Howl({
          src: ["Sounds/mo_" + No[i] + ".mp3"],
          preload: true,
          volume: 1.0,
          loop: false,
          autoplay: false,
          onload: () => {
            console.log(`木琴音源 ${No[i]} 読み込み完了`);
            resolve();
          },
          onloaderror: (id, error) => {
            console.warn(`木琴音源 ${No[i]} 読み込みエラー:`, error);
            resolve();
          }
        });
      }));
...
function createMokkinKeyboard(audioSources) {
  const No = [2, 4, 6, 0, 9, 11, 0, 14, 16, 18, 0, 21, 23, 0, 26, 1, 3, 5, 7, 8, 10, 12, 13, 15, 17, 19, 20, 22, 24, 25, 27];
  const sound_data = [
    "#ﾌｧ/♭ｿ",
    "#ｿ/♭ﾗ",
